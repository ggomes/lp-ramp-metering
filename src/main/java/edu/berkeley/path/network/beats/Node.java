...
package edu.berkeley.path.network.beats;

public class Node extends jaxb.Node {
	...
	protected void populate(Network myNetwork) {
		...
		if(getOutputs()!=null){
			...
			for(int i=0;i<nOut;i++){
				jaxb.Output output = getOutputs().getOutput().get(i);
		...
		if(getInputs()!=null){
			...
			for(int i=0;i<nIn;i++){
				jaxb.Input input = getInputs().getInput().get(i);
